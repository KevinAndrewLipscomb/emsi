...
sub get_command_out {
	...
	block: {
		...
		local $/ = undef();
		print "<p><b>Debug Status:</b> running command '$command'</p>\n" if ($b_verbose);
		$output = `$command`;

		if ((-e $temp) and (-e $temp)) {
			my ($err, $stderr) = &ReadFileL($temp);
			if ($b_verbose) {
				print "<p><b>Debug Status:</b> STDERR output follows:</p><h2><pre>" . &he($stderr) . "</pre></h2><p>~ end of STDERR output</p>\n";
				}
			}
		close(STDERR);
		unlink($temp) if (-e $temp);
		...
		if ($b_verbose) {
			print "<p><b>Debug Status:</b> STDOUT output follows:</p><h2><pre>" . &he($output) . "</pre></h2><p>~ end of STDOUT output</p>\n";
			unless (length($output)) {
				print "<p>(The captured STDOUT output was a string of zero length.)</p>\n";
				}
			}
		}
	return $output;
...
sub convert_pdf_to_text {
	...
	Err: {
		...
		my $headtext = &get_command_out( "$private{'pdf utility folder'}pdfinfo $tempname", $FORM{'debug'} );
		my $body = &get_command_out( "$private{'pdf utility folder'}pdftotext -raw $tempname -", $FORM{'debug'} );
	...
	foreach (split(m!\015|\012!, $RobotText)) {
		if (m!^user-agent:([^\#]*)!i) {
			...
			if ($forbidden_path eq '') {
				...
				if ((not $err) and ($clean =~ m!^http://virtual(.+)$!)) {
...
sub parse_html_ex {
	...
	if (($b_SaveLinks) or ($Rules{'index links'})) {
		...
		my $hostname = '';
		$hostname = quotemeta($1) if ($URL =~ m!^http://(.*?)/!);
...
sub pagedata_from_file {
	...
	Err: {
		...
		if ($file =~ m!\.pdf$!i) {
			...
			if (($Rules{'max characters: file'}) and (length($text) >  $Rules{'max characters: file'})) {
				$text = substr($text, 0, $Rules{'max characters: file'});
				}
			}
...
sub create_file_list {
	...
	Err: {
		...
		BuildTempList: {
			...
			if ((not $Rules{'crawler: rogue'}) and ($params{'base_url'} =~ m!^http://([^\/]+)(.*)$!)) { # c:rogue added 0062
				my $path = $2;
...
sub _handle_folder {
	...
	Err: {
		...
		&ppstr(89,&he($abs_folder_path)) if ($b_verbose);
		...
		while (defined($_ = readdir(DIR))) {
			next if (m!^\.\.?$!); # skip the . and .. entries
		...
		FolderEntry: foreach (sort @entries) {
			...
			elsif ($lc_ext !~ m! $qm_lc_extension !) {
	...
	Err: {
		...
		foreach (split(m!$separ!s, &Trim($FileText))) {
			...
			if ($data{'enabled'}) {
				...
				if ($record_err_msg) {
					&ppstr(53, $str[92] . ' - ' . $record_err_msg );
					next;
					}
				}
